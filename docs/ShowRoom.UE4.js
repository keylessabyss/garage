...
console.log("Emscripten version: E:/UE4_KeylessAbyss/UnrealEngine-4.27-html5-es3/Engine/Platforms/HTML5/Build/emsdk/emsdk-4.0.12/upstream/emscripten");
console.log("Emscripten configuration: E:/UE4_KeylessAbyss/UnrealEngine-4.27-html5-es3/Engine/Platforms/HTML5/Build/emsdk/emsdk-4.0.12/.emscripten");
...
var Module = {
	...
	UE4_indexedDBVersion: 202602151518, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = sessionStorageCommandLine != null ? [sessionStorageCommandLine] : ['../../../ShowRoom/ShowRoom.uproject','-stdout',];
...
//	}
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('ShowRoom'+dataVariant+'.data.js'));
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('ShowRoom'+dataVariant+'.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('ShowRoom'+dataVariant+'.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
